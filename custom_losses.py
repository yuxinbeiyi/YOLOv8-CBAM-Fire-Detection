...
class EIouLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        aspect_ratio = 4 / (math.pi ** 2) * (torch.atan(target_w / (target_h + self.eps)) - torch.atan(pred_w / (pred_h + self.eps))).pow(2)
...
class QualityFocalLoss(nn.Module):
    ...
    def forward(self, pred, target, quality_label):
        ...
        # 正样本损失
        pos_loss = quality_label * scale_factor.abs().pow(self.beta) * F.binary_cross_entropy_with_logits(
            pred, torch.ones_like(pred) * quality_label, reduction='none'
        )
        
        # 负样本损失
        neg_loss = (1 - quality_label) * scale_factor.abs().pow(self.beta) * F.binary_cross_entropy_with_logits(
