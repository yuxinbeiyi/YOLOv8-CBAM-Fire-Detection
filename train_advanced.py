...
def compare_loss_functions():
    """比较不同损失函数的效果."""
    loss_configs = [
        {"name": "baseline", "bbox_loss": "ciou", "cls_loss": "bce", "obj_loss": "bce"},
        {"name": "eiou_only", "bbox_loss": "eiou", "cls_loss": "bce", "obj_loss": "bce"},
        {"name": "eiou_focal", "bbox_loss": "eiou", "cls_loss": "focal", "obj_loss": "bce"},
        {"name": "eiou_varifocal", "bbox_loss": "eiou", "cls_loss": "varifocal", "obj_loss": "bce"},
    ]
    ...
    for config in loss_configs:
        ...
        results[config["name"]] = {"map50": train_results.box.map50, "map": train_results.box.map, "model": model}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bbox-loss", type=str, default="eiou", choices=["ciou", "diou", "giou", "eiou"], help="边界框损失函数"
    )
    parser.add_argument(
        "--cls-loss", type=str, default="bce", choices=["bce", "focal", "varifocal"], help="分类损失函数"
    )
    ...
    if args.mode == "train":
        # 单次训练
        loss_config = {"bbox_loss": args.bbox_loss, "cls_loss": args.cls_loss, "obj_loss": "bce"}

        results, model = train_with_custom_loss(epochs=args.epochs, batch_size=args.batch, loss_config=loss_config)
